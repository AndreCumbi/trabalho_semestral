...
function Usuario(){
    ...
    useEffect(()=>{
        axios.get(`http://localhost:8081/docs/${nuid}`)
        .then(res => setDocs(res.data))
        .catch(err => console.log(err))
    }, [nuid])

    useEffect(()=>{
        axios.get(`http://localhost:8081/nome/${nuid}`)
                    ...
                    <tbody>
                            {
                                ...
                                <td>{bi.num}</td>
                                <td>
                                <Link to={`detalhesbi/${bi.num}`} className="btn btn-primary">Detalhes</Link>
                                </td>
                                </tr>}{
                                docs.map((data, i) => (
                                    <tr key = {i}>
                                        ...
                                        <td>{data.cod_doc}</td>
                                        <td>
                                        <Link to={`detalhes/${data.cod_doc}`} className="btn btn-primary">Detalhes</Link>
                                        </td>
                                    </tr>
                                ))
                            }
